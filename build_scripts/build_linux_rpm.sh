...
if [ ! "$1" ]; then
  echo "This script requires either amd64 of arm64 as an argument"
	...
	DIR_NAME="scam-blockchain-linux-x64"
else
	#PLATFORM="$1"
	DIR_NAME="scam-blockchain-linux-arm64"
fi
...
electron-packager . scam-blockchain --asar.unpack="**/daemon/**" --platform=linux \
...
if [ "$REDHAT_PLATFORM" = "x86_64" ]; then
	echo "Create scam-blockchain-$SCAM_INSTALLER_VERSION.rpm"
  electron-installer-redhat --src dist/$DIR_NAME/ --dest final_installer/ \
  --arch "$REDHAT_PLATFORM" --options.version $SCAM_INSTALLER_VERSION \
  --license ../LICENSE
  LAST_EXIT_CODE=$?
  if [ "$LAST_EXIT_CODE" -ne 0 ]; then
