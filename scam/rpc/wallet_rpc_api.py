...
from scam.cmds.init_funcs import check_keys
from scam.consensus.block_rewards import calculate_base_farmer_reward
from scam.pools.pool_wallet import PoolWallet
from scam.pools.pool_wallet_info import create_pool_state, FARMING_TO_POOL, PoolWalletInfo, PoolState
from scam.protocols.protocol_message_types import ProtocolMessageTypes
from scam.server.outbound_message import NodeType, make_msg
...
from scam.util.keychain import bytes_to_mnemonic, generate_mnemonic
from scam.util.path import path_from_root
from scam.util.ws_message import WsRpcMessage, create_payload_dict
...
from scam.wallet.derive_keys import master_sk_to_farmer_sk, master_sk_to_pool_sk, master_sk_to_wallet_sk
...
from scam.wallet.util.backup_utils import download_backup, get_backup_info, upload_backup
from scam.wallet.util.trade_utils import trade_record_to_dict
from scam.wallet.util.transaction_type import TransactionType
from scam.wallet.util.wallet_types import WalletType
from scam.wallet.wallet_info import WalletInfo
from scam.wallet.wallet_node import WalletNode
...
class WalletRpcApi:
    ...
    async def _state_changed(self, *args) -> List[WsRpcMessage]:
        ...
        return [create_payload_dict("state_changed", data, "scam_wallet", "wallet_ui")]
    ...
    async def _check_key_used_for_rewards(
        ...
        farmer_target = config["farmer"].get("scm_target_address")
        pool_target = config["pool"].get("scm_target_address")
        found_farmer = False
        found_pool = False
    ...
    async def create_new_wallet(self, request: Dict):
        ...
        if request["wallet_type"] == "cc_wallet":
            ...
            if request["mode"] == "new":
                ...
                from scam.pools.pool_wallet_info import initial_pool_state_from_dict
