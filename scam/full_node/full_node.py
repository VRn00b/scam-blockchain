...
import scam.server.ws_connection as ws  # lgtm [py/import-and-import-from]
...
from scam.consensus.pot_iterations import calculate_sp_iters
...
from scam.full_node.weight_proof import WeightProofHandler
from scam.protocols import farmer_protocol, full_node_protocol, timelord_protocol, wallet_protocol
from scam.protocols.full_node_protocol import (
    RejectBlocks,
    RequestBlocks,
    RespondBlock,
    RespondBlocks,
...
from scam.server.node_discovery import FullNodePeers
from scam.server.outbound_message import Message, NodeType, make_msg
from scam.server.server import ScamServer
...
from scam.types.blockchain_format.vdf import CompressibleVDFField, VDFInfo, VDFProof
...
from scam.util.errors import ConsensusError, Err
from scam.util.ints import uint8, uint32, uint64, uint128
from scam.util.path import mkdir, path_from_root
from scam.util.safe_cancel_task import cancel_task_safe
from scam.util.profiler import profile_task
...
class FullNode:
    ...
    def set_server(self, server: ScamServer):
        self.server = server
        dns_servers = []
        try:
        ...
        elif self.config["port"] == 9777:
            # If `dns_servers` misses from the `config`, hardcode it if we're running mainnet.
            dns_servers.append("dns-introducer.scamnetwork.net")
        try:
    ...
    async def short_sync_batch(self, peer: ws.WSScamConnection, start_height: uint32, target_height: uint32) -> bool:
        """
    ...
    async def short_sync_backtrack(
        self, peer: ws.WSScamConnection, peak_height: uint32, target_height: uint32, target_unf_hash: bytes32
    ):
        """
    ...
    async def new_peak(self, request: full_node_protocol.NewPeak, peer: ws.WSScamConnection):
        """
    ...
    async def send_peak_to_timelords(
        self, peak_block: Optional[FullBlock] = None, peer: Optional[ws.WSScamConnection] = None
    ):
        """
    ...
    async def on_connect(self, connection: ws.WSScamConnection):
        """
    ...
    def on_disconnect(self, connection: ws.WSScamConnection):
    ...
    async def peak_post_processing(
        self, block: FullBlock, record: BlockRecord, fork_height: uint32, peer: Optional[ws.WSScamConnection]
    ):
        """
    ...
    async def respond_block(
        ...
        peer: Optional[ws.WSScamConnection] = None,
    ) -> Optional[Message]:
        """
    ...
    async def respond_unfinished_block(
        ...
        peer: Optional[ws.WSScamConnection],
        ...
        if farmed_block is True:
            ...
            self.log.info(
                ...
                f"Pool pk {encode_puzzle_hash(block.foliage.foliage_block_data.pool_target.puzzle_hash, 'scm')}, "
                f"validation time: {validation_time}, "
                f"cost: {block.transactions_info.cost if block.transactions_info else 'None'}"
                f"{percent_full_str}"
            )
    ...
    async def new_infusion_point_vdf(
        self, request: timelord_protocol.NewInfusionPointVDF, timelord_peer: Optional[ws.WSScamConnection] = None
    ) -> Optional[Message]:
