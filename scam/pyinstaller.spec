...
ROOT = pathlib.Path(importlib.import_module("scam").__file__).absolute().parent.parent


def solve_name_collision_problem(analysis):
    """
    There is a collision between the `scam` file name (which is the executable)
    and the `scam` directory, which contains non-code resources like `english.txt`.
    We move all the resources in the zipped area so there is no
    need to create the `scam` directory, since the names collide.
...
version_data = copy_metadata(get_distribution("scam-blockchain"))[0]
...
# TODO: collapse all these entry points into one `scam_exec` entrypoint that accepts the server as a parameter

entry_points = ["scam.cmds.scam"] + [f"scam.server.start_{s}" for s in SERVERS]
...
if THIS_IS_WINDOWS:
    entry_points.extend(["aiohttp", "scam.util.bip39"])

if THIS_IS_WINDOWS:
    scam_mod = importlib.import_module("scam")
    dll_paths = ROOT / "*.dll"
...
datas.append((f"{ROOT}/scam/util/english.txt", "scam/util"))
datas.append((f"{ROOT}/scam/util/initial-config.yaml", "scam/util"))
datas.append((f"{ROOT}/scam/wallet/puzzles/*.hex", "scam/wallet/puzzles"))
datas.append((f"{ROOT}/scam/ssl/*", "scam/ssl"))
...
add_binary("scam", f"{ROOT}/scam/cmds/scam.py", COLLECT_ARGS)
add_binary("daemon", f"{ROOT}/scam/daemon/server.py", COLLECT_ARGS)

for server in SERVERS:
    add_binary(f"start_{server}", f"{ROOT}/scam/server/start_{server}.py", COLLECT_ARGS)
