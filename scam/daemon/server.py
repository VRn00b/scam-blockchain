...
from scam.cmds.init_funcs import scam_init
from scam.daemon.windows_signal import kill
from scam.server.server import ssl_context_for_root, ssl_context_for_server
from scam.ssl.create_ssl import get_mozilla_ca_crt
from scam.util.scam_logging import initialize_logging
from scam.util.config import load_config
from scam.util.json_util import dict_to_json_str
from scam.util.path import mkdir
from scam.util.service_groups import validate_service
from scam.util.setproctitle import setproctitle
from scam.util.ws_message import WsRpcMessage, create_payload, format_response
...
try:
    ...
    print("Error: Make sure to run . ./activate from the project folder before starting Scam.")
    quit()
...
service_plotter = "scam plots create"
...
if getattr(sys, "frozen", False):
    name_map = {
        "scam": "scam",
        "scam_wallet": "start_wallet",
        "scam_full_node": "start_full_node",
        "scam_harvester": "start_harvester",
        "scam_farmer": "start_farmer",
        "scam_introducer": "start_introducer",
        "scam_timelord": "start_timelord",
        "scam_timelord_launcher": "timelord_launcher",
        "scam_full_node_simulator": "start_simulator",
    }
...
class WebSocketServer:
    ...
    async def exit(self) -> Dict[str, Any]:
        ...
        log.info("scam daemon exiting in 5 seconds")
...
def launch_plotter(root_path: Path, service_name: str, service_array: List[str], id: str):
    # we need to pass on the possibly altered SCAM_ROOT
    os.environ["SCAM_ROOT"] = str(root_path)
...
def launch_service(root_path: Path, service_command) -> Tuple[subprocess.Popen, Path]:
    ...
    log.debug(f"Launching service with SCAM_ROOT: {os.environ['SCAM_ROOT']}")
