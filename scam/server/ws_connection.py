...
from scam.cmds.init_funcs import scam_full_version_str
from scam.protocols.protocol_message_types import ProtocolMessageTypes
from scam.protocols.shared_protocol import Capability, Handshake
...
from scam.util.ints import uint8, uint16

# Each message is prepended with LENGTH_BYTES bytes specifying the length
from scam.util.network import class_for_type, is_localhost

# Max size 2^(8*4) which is around 4GiB
LENGTH_BYTES: int = 4


class WSScamConnection:
    """
    ...
    async def perform_handshake(self, network_id: str, protocol_version: str, server_port: int, local_type: NodeType):
        if self.is_outbound:
            ...
            if inbound_handshake.network_id != 'scam-' + network_id:
