...
from scam.protocols import full_node_protocol, introducer_protocol
...
class FullNodeDiscovery:
    ...
    async def on_connect(self, peer: ws.WSScamConnection):
        if (
    ...
    async def update_peer_timestamp_on_message(self, peer: ws.WSScamConnection):
        if (
            peer.is_outbound
            and peer.peer_server_port is not None
            and peer.connection_type is NodeType.FULL_NODE
    ...
    async def _introducer_client(self):
        ...
        async def on_connect(peer: ws.WSScamConnection):
