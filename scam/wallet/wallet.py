...
from scam.wallet.puzzles.puzzle_utils import (
...
from scam.wallet.secret_key_store import SecretKeyStore
from scam.wallet.sign_coin_solutions import sign_coin_solutions
...
class Wallet:
    ...
    async def create_spend_bundle_relative_scam(self, scam_amount: int, exclude: List[Coin]) -> SpendBundle:
        list_of_solutions = []
        utxos = None
        ...
        if scam_amount < 0:
            utxos = await self.select_coins(abs(scam_amount), exclude)
        else:
        ...
        scam_amount = spend_value + scam_amount
        ...
        for coin in utxos:
            ...
            if output_created is None:
                ...
                primaries = [{"puzzlehash": newpuzhash, "amount": scam_amount}]
