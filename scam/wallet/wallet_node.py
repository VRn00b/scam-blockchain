...
from scam.pools.pool_puzzles import SINGLETON_LAUNCHER_HASH
from scam.protocols import wallet_protocol
from scam.protocols.full_node_protocol import RequestProofOfWeight, RespondProofOfWeight
from scam.protocols.protocol_message_types import ProtocolMessageTypes
from scam.protocols.wallet_protocol import (
...
from scam.util.errors import Err, ValidationError
from scam.util.ints import uint32, uint128
from scam.util.keychain import Keychain
from scam.util.lru_cache import LRUCache
from scam.util.merkle_set import MerkleSet, confirm_included_already_hashed, confirm_not_included_already_hashed
from scam.util.path import mkdir, path_from_root
from scam.wallet.block_record import HeaderBlockRecord
from scam.wallet.derivation_record import DerivationRecord
from scam.wallet.settings.settings_objects import BackupInitialized
from scam.wallet.transaction_record import TransactionRecord
from scam.wallet.util.backup_utils import open_backup_file
from scam.wallet.util.wallet_types import WalletType
from scam.wallet.wallet_action import WalletAction
from scam.wallet.wallet_blockchain import ReceiveBlockResult
...
class WalletNode:
    ...
    def get_key_for_fingerprint(self, fingerprint: Optional[int]) -> Optional[PrivateKey]:
        ...
        if len(private_keys) == 0:
            self.log.warning("No keys present. Create keys with the UI, or with the 'scam keys' program.")
            return None
    ...
    async def on_connect(self, peer: WSScamConnection):
        if self.wallet_state_manager is None or self.backup_initialized is False:
            return None
    ...
    async def complete_blocks(self, header_blocks: List[HeaderBlock], peer: WSScamConnection):
    ...
    async def new_peak_wallet(self, peak: wallet_protocol.NewPeakWallet, peer: WSScamConnection):
        if self.wallet_state_manager is None:
            return
    ...
    async def _sync(self) -> None:
        ...
        peers: List[WSScamConnection] = self.server.get_full_node_connections()
        if len(peers) == 0:
    ...
    async def get_additions(
        self, peer: WSScamConnection, block_i, additions: Optional[List[bytes32]], get_all_additions: bool = False
    ) -> Optional[List[Coin]]:
    ...
    async def get_removals(
        self, peer: WSScamConnection, block_i, additions, removals, request_all_removals=False
    ) -> Optional[List[Coin]]:
        assert self.wallet_state_manager is not None
