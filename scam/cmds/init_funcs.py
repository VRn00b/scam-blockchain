...
from scam.ssl.create_ssl import generate_ca_signed_cert, get_scam_ca_crt_key, make_ca_cert
from scam.util.bech32m import encode_puzzle_hash
from scam.util.config import (
    create_default_scam_config,
    initial_config_file,
    load_config,
    save_config,
...
from scam.util.keychain import Keychain
from scam.util.path import mkdir
from scam.wallet.derive_keys import master_sk_to_pool_sk, master_sk_to_wallet_sk
...
def check_keys(new_root: Path) -> None:
    ...
    if len(all_sks) == 0:
        print("No keys are present in the keychain. Generate them with 'scam keys generate'")
        return None

    config: Dict = load_config(new_root, "config.yaml")
    pool_child_pubkeys = [master_sk_to_pool_sk(sk).get_g1() for sk, _ in all_sks]
    all_targets = []
    stop_searching_for_farmer = "scm_target_address" not in config["farmer"]
    stop_searching_for_pool = "scm_target_address" not in config["pool"]
    ...
    for i in range(number_of_ph_to_search):
        ...
        for sk, _ in all_sks:
            ...
            if all_targets[-1] == config["farmer"].get("scm_target_address"):
                stop_searching_for_farmer = True
            if all_targets[-1] == config["pool"].get("scm_target_address"):
                stop_searching_for_pool = True

    # Set the destinations
    if "scm_target_address" not in config["farmer"]:
        print(f"Setting the scm destination address for coinbase fees reward to {all_targets[0]}")
        config["farmer"]["scm_target_address"] = all_targets[0]
    elif config["farmer"]["scm_target_address"] not in all_targets:
        print(
            f"WARNING: using a farmer address which we don't have the private"
            f" keys for. We searched the first {number_of_ph_to_search} addresses. Consider overriding "
            f"{config['farmer']['scm_target_address']} with {all_targets[0]}"
        )

    if "pool" not in config:
        config["pool"] = {}
    if "scm_target_address" not in config["pool"]:
        print(f"Setting the scm destination address for coinbase reward to {all_targets[0]}")
        config["pool"]["scm_target_address"] = all_targets[0]
    elif config["pool"]["scm_target_address"] not in all_targets:
        print(
            f"WARNING: using a pool address which we don't have the private"
            f" keys for. We searched the first {number_of_ph_to_search} addresses. Consider overriding "
            f"{config['pool']['scm_target_address']} with {all_targets[0]}"
        )
...
def create_all_ssl(root: Path):
    ...
    scam_ca_crt, scam_ca_key = get_scam_ca_crt_key()
    scam_ca_crt_path = ca_dir / "scam_ca.crt"
    scam_ca_key_path = ca_dir / "scam_ca.key"
    scam_ca_crt_path.write_bytes(scam_ca_crt)
    scam_ca_key_path.write_bytes(scam_ca_key)
    ...
    scam_ca_crt, scam_ca_key = get_scam_ca_crt_key()
    generate_ssl_for_nodes(ssl_dir, scam_ca_crt, scam_ca_key, False, overwrite=False)
...
def init(create_certs: Optional[Path], root_path: Path):
    if create_certs is not None:
        if root_path.exists():
            ...
            if scam_init(root_path) == 0 and root_path.exists():
                return init(create_certs, root_path)

            print(f"** {root_path} was not created. Exiting **")
            return -1
    else:
        return scam_init(root_path)


def scam_version_number() -> Tuple[str, str, str, str]:
...
def scam_minor_release_number():
    res = int(scam_version_number()[2])
    print(f"Install release number: {res}")
    return res


def scam_full_version_str() -> str:
    major, minor, patch, dev = scam_version_number()
    return f"{major}.{minor}.{patch}{dev}"


def scam_init(root_path: Path):
    if os.environ.get("SCAM_ROOT", None) is not None:
        print(
            f"warning, your SCAM_ROOT is set to {os.environ['SCAM_ROOT']}. "
            f"Please unset the environment variable and run scam init again\n"
            f"or manually migrate config.yaml"
        )

    print(f"Scam directory {root_path}")
    if root_path.is_dir() and Path(root_path / "config" / "config.yaml").exists():
        # This is reached if SCAM_ROOT is set, or if user has run scam init twice
        # before a new update.
    ...
    check_keys(root_path)
    print("")
    print("To see your keys, run 'scam keys show --show-mnemonic-seed'")
