...
from scam.protocols.shared_protocol import Handshake
from scam.server.outbound_message import make_msg, Message
from scam.server.rate_limits import RateLimiter
from scam.server.server import ssl_context_for_client
from scam.server.ws_connection import WSScamConnection
from scam.types.peer_info import PeerInfo
from scam.util.ints import uint16, uint64
...
class TestDos:
    ...
    async def test_large_message_disconnect_and_ban(self, setup_two_nodes):
        ...
        ssl_context = ssl_context_for_client(
            server_2.scam_ca_crt_path, server_2.scam_ca_key_path, server_2.p2p_crt_path, server_2.p2p_key_path
        )
    ...
    async def test_spam_tx(self, setup_two_nodes):
        ...
        ws_con: WSScamConnection = list(server_1.all_connections.values())[0]
        ws_con_2: WSScamConnection = list(server_2.all_connections.values())[0]
