...
import scamEnvironment from '../util/scamEnvironment';
import scamConfig from '../util/config';
...
if (!handleSquirrelEvent()) {
  ...
  const ensureCorrectEnvironment = () => {
    ...
    if (!scamEnvironment.guessPackaged() && !('VIRTUAL_ENV' in process.env)) {
  ...
  if (ensureSingleInstance() && ensureCorrectEnvironment()) {
    ...
    const createWindow = () => {
      ...
      if (!scamConfig.manageDaemonLifetime()) {
      ...
      mainWindow.on('close', (e) => {
        ...
        if (decidedToClose || !scamConfig.manageDaemonLifetime()) {
          return;
        }
        e.preventDefault();
        if (!isClosing) {
    ...
    const appReady = async () => {
      ...
      // if the daemon isn't local we aren't going to try to start/stop it
      if (scamConfig.manageDaemonLifetime()) {
        scamEnvironment.startScamDaemon();
      }
    };
  ...
  const getMenuTemplate = () => {
    const template = [
      ...
      },
      {
        ...
        submenu: [
          {
            label: i18n._(/* i18n */ { id: 'Scam Blockchain Wiki' }),
            click: () => {
              openExternal(
                'https://github.com/vrn00b/scam-blockchain/wiki',
              );
            },
          },
          {
            ...
            click: () => {
              openExternal(
                'https://github.com/vrn00b/scam-blockchain/wiki/FAQ',
              );
            },
          },
          {
            ...
            click: () => {
              openExternal(
                'https://github.com/vrn00b/scam-blockchain/releases',
              );
            },
          },
          {
            ...
            click: () => {
              openExternal(
                'https://github.com/vrn00b/scam-blockchain/blob/master/CONTRIBUTING.md',
              );
            },
          },
          {
          ...
          },
          {
            ...
            click: () => {
              openExternal(
                'https://github.com/vrn00b/scam-blockchain/issues',
              );
            },
          },
          {
            ...
            click: () => {
              openExternal('https://keybase.io/team/scam_network.public');
            },
          },
          {
            label: i18n._(/* i18n */ { id: 'Follow on Twitter' }),
            click: () => {
              openExternal('https://twitter.com/scam_project');
            },
          },
        ],
      },
    ...
    if (process.platform === 'darwin') {
      // Scam Blockchain menu (Mac)
      template.unshift({
        label: i18n._(/* i18n */ { id: 'Scam' }),
        submenu: [
          {
            label: i18n._(/* i18n */ { id: 'About Scam Blockchain' }),
            click: () => {
              openAbout();
            },
          },
