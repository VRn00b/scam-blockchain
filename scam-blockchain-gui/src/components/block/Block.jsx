...
export default function Block() {
  ...
  const poolReward = mojo_to_scam(calculatePoolReward(blockRecord.height));
  const baseFarmerReward = mojo_to_scam(
    calculateBaseFarmerReward(blockRecord.height),
  );

  const scamFees = blockRecord.fees
    ? mojo_to_scam(BigInt(blockRecord.fees))
    : '';
  ...
  const rows = [
    ...
    },
    {
      ...
      value: (
        <Link
          target="_blank"
          href={`https://www.scamexplorer.com/blockchain/puzzlehash/${blockRecord.farmer_puzzle_hash}`}
        >
          {currencyCode
            ? toBech32m(
    ...
    },
    {
      ...
      value: (
        <Link
          target="_blank"
          href={`https://www.scamexplorer.com/blockchain/puzzlehash/${blockRecord.pool_puzzle_hash}`}
        >
          {currencyCode
            ? toBech32m(
    ...
    },
    {
      ...
      value: scamFees ? `${scamFees} ${currencyCode}` : '',
      tooltip: (
        <Trans>
    ...
    <LayoutMain title={<Trans>Block</Trans>}>
      <Card
        title={
          <BlockTitle>
            <Trans>
              Block at height {blockRecord.height} in the Scam blockchain
            </Trans>
          </BlockTitle>
        }
        action={
