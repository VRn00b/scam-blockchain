...
import { AlertDialog, Card, Flex } from '@scam/core';
import {
  send_transaction,
...
const IncompleteCard = (props) => {
  ...
  function submit() {
    ...
    const scamper_input = ip_parsed.limit;
    ...
    const scamper_value = Number.parseInt(Number(scamper_input));
        ...
        <Grid item xs={12}>
          <Box display="flex">
            ...
            <Box flexGrow={1}>
              <Typography variant="subtitle1">
                <Trans>
                  Spending Limit (scam per interval):{' '}
                  {mojo_to_scam_string(limit)}
                </Trans>
              </Typography>
            </Box>
          </Box>
...
const SendCard = (props) => {
  ...
  function send() {
    ...
    const amount = scam_to_mojo(amount_input.value);
    const fee = scam_to_mojo(fee_input.value);
