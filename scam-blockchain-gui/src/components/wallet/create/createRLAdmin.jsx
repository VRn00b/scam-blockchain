...
import { AlertDialog } from '@scam/core';
import {
  Typography,
  Button,
  Box,
...
import { scam_to_mojo } from '../../../util/scam';
...
export const CreateRLAdminWallet = () => {
  ...
  function create() {
    ...
    if (
      scamper_input.value === '' ||
      Number(scamper_input.value) === 0 ||
      !Number(scamper_input.value) ||
      isNaN(Number(scamper_input.value))
    ) {
      dispatch(
        openDialog(
          <AlertDialog>
    ...
    const scamper = scam_to_mojo(scamper_input.value);
    const scamper_value = Number.parseInt(Number(scamper));
    ...
    // var fee = scam_to_mojo(fee_input.value);
