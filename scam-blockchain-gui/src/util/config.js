...
global.daemon_rpc_ws = `wss://${self_hostname}:9701`;
...
function loadConfig(net) {
  try {
    // check if SCAM_ROOT is set. it overrides 'net'
    const config_root_dir =
      'SCAM_ROOT' in process.env
        ? process.env.SCAM_ROOT
        : path.join(os.homedir(), '.scam', net);
    ...
    const daemon_port = lodash.get(config, 'ui.daemon_port', 9701); // jshint ignore:line
