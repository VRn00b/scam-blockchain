...
export function calculateBaseFarmerReward(height: number): Big {
  if (height === 0) {
    return MOJO_PER_SCAM.times(3000000).times(8 / 8);
  }
  if (height < 3 * BLOCKS_PER_YEAR) {
    return MOJO_PER_SCAM.times(10).times(8 / 8);
  }
  if (height < 6 * BLOCKS_PER_YEAR) {
    return MOJO_PER_SCAM.times(5).times(8 / 8);
  }
  if (height < 9 * BLOCKS_PER_YEAR) {
    return MOJO_PER_SCAM.times(2.5).times(8 / 8);
  }
  if (height < 12 * BLOCKS_PER_YEAR) {
    return MOJO_PER_SCAM.times(1.25).times(8 / 8);
  }

  return MOJO_PER_SCAM.times(0.625).times(8 / 8);
}
