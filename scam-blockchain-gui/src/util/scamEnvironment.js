...
const getScamVersion = () => {
  let version = null;
  const exePath = getExecutablePath('scam');
  // first see if we can get a scam exe in a standard location relative to where we are
  try {
    ...
    // that didn't work, let's try as if we're in the venv or scam is on the path
    try {
      version = child_process
